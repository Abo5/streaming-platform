...
export function ContentRow({ title, viewMoreLink, items }: ContentRowProps) {
      ...
      <div className="relative">
        ...
        >
          {items.map((item) => (
            <div
              key={item.id}
              className="relative flex-shrink-0 w-[180px] md:w-[220px] group/item"
            ...
            >
              <Link href={`/${item.type}/${item.id}`}>
                <div className="relative aspect-[2/3] rounded-lg overflow-hidden">
                  ...
                  {item.isLive && (
                    <div className="absolute top-2 right-2 bg-red-600 text-white text-xs px-2 py-0.5 rounded-full flex items-center z-30">
